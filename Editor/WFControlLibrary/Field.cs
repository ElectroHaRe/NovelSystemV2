...
namespace WFControlLibrary
{
    public partial class Field : UserControl
    {
        ...
        private void ChangeFocus(FieldElement item)
        {
            ...
            Focus = tree.CurrentScene as FieldElement;
        }
        ...
        private void UpdateWorldParams()
        {
            ...
            Parallel.ForEach(tree.GetAllScenes(), item =>
            {
                {
        ...
        public void FocusOnPoint(Point worldPoint)
        {
            ...
            foreach (var item in tree.GetAllScenes())
            {
        ...
        #region TreeWrapper
        public void AddLink(FieldElement from, string text, FieldElement to)
        {
            tree.AddLink(from, text, to);
        }
        public void RemoveLink(FieldElement source, FieldElement linkedItem)
        {
            tree.RemoveLink(source, linkedItem);
        }
        public void RemoveLink(FieldElement source, string linkedText)
        {
            tree.RemoveLink(source, linkedText);
        }
        public void SetFocus(FieldElement item)
        {
            tree.SetCurrent(item);
            Focus = item;
        ...
        private void Field_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var source in tree.GetAllScenes())
            {
                if ((source as IFieldElement).Highlight)
                    DrawHighlightOf(e.Graphics, source as FieldElement);

                foreach (var destination in tree.GetAllLinkedScenesFor(source))
                {
